...
약 54M의 pre-training data로 학습한 모델로 fine-tuning한 결과, 약 83%의 정확도 달성하였다. 이는 일반적인 말뭉치만으로는 포착하기 어려운 실적 발표 표현, 가격 변동 서술, 거시경제 이벤트, 금융 전문 용어 등에 따른 패턴을 충분히 모델 파라미터에 내재화함으로써 합리적인 수준의 성능을 달성할 수 있음을 보여준다. 
